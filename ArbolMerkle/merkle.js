  ...
  genHash(tmp, n) { // postorder
    if (tmp != null) {
      ...
      if (tmp.left == null && tmp.right == null) {
        ...
        tmp.hash = Sha256.hash((tmp.left.value).toString())
      } else {
        tmp.hash = Sha256.hash(Sha256.hash(tmp.left.hash.toString())+Sha256.hash(tmp.right.hash.toString()))
      }      
    }
  }
  ...
  preorder(tmp) {
    if (tmp != null) {
      if (tmp instanceof DataNode) {
        ...
        let nodoTempo=tmp.value
        console.log("DATO: "+nodoTempo)
...
graficar(){
    //globalThis.nodosBinario=""
    //globalThis.conexionesBinario =""
    ...
    if (tmp != null) {
      if (tmp.left != null){
        if (tmp.left instanceof DataNode) {
          //this.dot += '"'+tmp.left.value+'" [color=gray] -> "'+tmp.hash+'";'
          conexionesBinario+=('"'+tmp.hash+'"  -> "'+ tmp.left.value+'"[arrowhead =null ];\n');
        }
      }
      if (tmp.left instanceof HashNode) {
        if (tmp.left != null){ 
          //this.dot += '"0x'+tmp.left.hash+'" -> "0x'+tmp.hash+'";'
        conexionesBinario+=('"' +tmp.hash  +'"' + " -> " +'"'+ tmp.left.hash+'"' + "[arrowhead =null ];\n")}
        if (tmp.right != null){ 
          //this.dot += '"0x'+tmp.right.hash+'" -> "0x'+tmp.hash+'";'
        conexionesBinario+=('"' +tmp.hash+'"' + " -> " +'"'+tmp.right.hash  +'"' + "[arrowhead =null ];\n")}
      }
...
function merkle() {
    ...
    //m.dot += '}'
    //return m.dot
    console.log(" hash 1: "+m.tophash.hash)
     m.graficar()

     m.add("maria")
     m.add("maria2")
      m.auth()
     console.log(" hash 2: "+m.tophash.hash)
